...
const CustomSelect: React.FC<{ label: string; value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; children: React.ReactNode }> = ({ label, value, onChange, children }) => (
    <div className="flex-1 min-w-[150px]">
        <label className="block text-sm font-medium text-[--color-accent-red] mb-1 uppercase tracking-wider font-orbitron">{label}</label>
        <div className="relative">
            <select
                value={value}
                onChange={onChange}
                className="w-full bg-[--color-bg-tertiary] border border-[--color-border-primary] text-[--color-text-primary] rounded-md py-2 pl-3 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-[--color-accent-red] focus:border-[--color-accent-red] transition-all duration-200 font-mono"
                style={{ boxShadow: 'inset 0 2px 4px rgba(0, 0, 0, 0.3)' }}
            >
                {children}
            </select>
            <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                <svg className="h-5 w-5 text-[--color-accent-red]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
...
const AgentControlPanel: React.FC<AgentControlPanelProps> = ({
  ...
  return (
    <div className="bg-[--color-bg-secondary]/80 border border-[--color-border-primary] rounded-lg p-4 md:p-6 shadow-lg backdrop-blur-md"
      ...
      <h2 className="text-lg font-orbitron text-[--color-accent-red] mb-4 border-b border-[--color-accent-red]/30 pb-2 uppercase tracking-wider"
          style={{ textShadow: '0 0 10px var(--color-glow-red)' }}>
        :: Mission Parameters ::
      </h2>
            ...
            <CustomSelect label="LLM Provider" value={selectedLLMProvider} onChange={(e) => onLLMProviderChange(e.target.value as LLMProvider)}>
             ...
             <div className="flex-1 min-w-[150px]">
                ...
                <input
                    ...
                    className="w-full bg-[--color-bg-tertiary] border border-[--color-border-primary] text-[--color-text-primary] rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-[--color-accent-red] focus:border-[--color-accent-red] transition-all duration-200 font-mono"
                    style={{ boxShadow: 'inset 0 2px 4px rgba(0, 0, 0, 0.3)' }}
                />
            </div>
        </div>
      ...
      {(availableTools.length > 0 || customTools.length > 0) && (
          <div className="mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 bg-[--color-bg-tertiary]/50 p-3 rounded-md border border-[--color-border-primary]"
                  ...
                  {allTools.map(tool => (
                      <label key={tool} className="flex items-center space-x-2 text-sm text-[--color-text-secondary] cursor-pointer hover:text-[--color-accent-red] transition-colors font-mono">
                          <input
                              type="checkbox"
                              checked={selectedTools.includes(tool)}
                              onChange={() => onToolsChange(tool)}
                              className="w-4 h-4 text-[--color-accent-red] bg-[--color-bg-tertiary] border-[--color-border-primary] rounded focus:ring-[--color-accent-red]/80 focus:ring-2"
